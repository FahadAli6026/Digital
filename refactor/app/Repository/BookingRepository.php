...
class BookingRepository extends BaseRepository
{
    ...
    public function store($user, $data)
    {
        ...
        $cuser = $user;
        $immediateTime = 5;
        $response = $this->processBookingData($data, $cuser, $immediateTime);
    ...
    private function validateBookingData($data)
    {
        ...
        if ($data['immediate'] == 'no') {
            $immediateFields = ['due_date', 'due_time', 'customer_phone_type', 'customer_physical_type'];
    ...
    private function processBookingData($data, $cuser, $immediateTime)
    {
        $response = [];
        $due = $this->calculateDueDate($data, $immediateTime);
        ...
        $data['will_expire_at'] = TeHelper::willExpireAt($due['due'], $data['b_created_at']);
    ...
    private function calculateDueDate($data, $immediateTime)
    {
        if ($data['immediate'] == 'yes') {
            $dueCarbon = now()->addMinute($immediateTime);
            return ['status' => 'success', 'due' => $dueCarbon->format('Y-m-d H:i:s')];
        } else {
    ...
    public function jobEnd($post_data = [])
    {
        ...
        $session_time = implode(' ', explode(':', $job->session_time));
        $data = compact('user', 'job', 'session_time') + ['for_text' => 'faktura'];
        ...
        $name = $user->name;
        $subject = 'Information om avslutad tolkning för bokningsnummer # ' . $job->id;
        $data = compact('user', 'job', 'session_time') + ['for_text' => 'lön'];
        $this->mailer->send($email, $name, $subject, 'emails.session-ended', $data);
                ...
                if ($job->id == $oneJob->id) {
                    ...
                    if ($job_for_translator == 'SpecificJob') {
                        ...
                        if ($job_checker != 'userCanNotAcceptJob') {
                            $translators = $this->isNeedToDelayPush($oneUser->id) ? $delpay_translator_array : $translator_array;
                            $translators[] = $oneUser;
                        }
                    }
                }
